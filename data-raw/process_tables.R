...
cites_parties <- h("data-raw", "cites_parties.tsv") %>%
  ...
  filter(
    !(country == "Slovakia" & former_code == TRUE),
...
countries <-
  ...
  tidyr::separate(.,
    col = code, into = c("code", "non_ISO_code"),
    sep = "1", fill = "right"
  ) %>%
  mutate(
    non_ISO_code = ifelse(is.na(non_ISO_code), FALSE, TRUE),
...
joined_parties <- full_join(cites_parties, countries,
  ...
  filter(
    !(country == "Former Serbia and Montenegro"),
