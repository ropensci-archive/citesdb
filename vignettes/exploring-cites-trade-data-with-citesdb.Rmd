...
```{r}
library(rcites)
spp <- spp_taxonconcept("Paphiopedilum appletonianum")
spc <- spp_cites_legislation(spp$general$id)
eu <- spp_eu_legislation(spp$general$id)
cites_shipments() %>% 
  filter(Taxon == "Paphiopedilum appletonianum") %>% 
  count(Year) %>% 
  collect() %>% 
