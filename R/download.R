...
cites_db_download <- function(tag = NULL,  destdir = tempdir(),
  ...
  zfile <- get_gh_release_file("ecohealthalliance/citesdb", tag_name = tag,
  ...
  suppressMessages(
  dbExecute(cites_db(),
            paste0("COPY OFFSET 2 INTO ", tblname, " FROM '",
                   temp_tsv,
                   "' USING DELIMITERS '\t','\n','\"' NULL as 'NA'"))
  )
...
cites_field_types <- c(id = "INTEGER", year = "INTEGER", appendix = "STRING", taxon = "STRING",
...
get_gh_release_file <- function(repo, tag_name = NULL, destdir = tempdir(),
  ...
  releases <- GET(paste0("https://api.github.com/repos/", repo, "/releases"),
  ...
  response <- GET(paste0(download_url, "?access_token=", Sys.getenv("GITHUB_PAT")),
